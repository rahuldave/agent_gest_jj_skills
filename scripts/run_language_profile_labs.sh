...
workspace="$(mktemp -d "${TMPDIR:-/tmp}/agent-gest-jj-language-labs.XXXXXX")"
...
finish_lab() {
  local dir="$1"
  run "$dir" jj describe -m "test: verify $(basename "$dir") language profile"
  run "$dir" jj status
}
...
echo "Running jj language profile labs in $workspace"
write_python_lab
write_typescript_lab
write_go_lab
write_rust_lab
echo "jj language profile labs passed"
