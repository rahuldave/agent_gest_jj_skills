...
project types in colocated jj/git repositories. They are examples, not golden
project generators. Use them to test the workflow in a new chat or clean
project, then delete the lab folders.
...
Go, and Rust/Cargo; initializes Git, jj, and local Gest in each one; composes the
...
- Git initialization.
- Colocated jj initialization with `jj git init --colocate`.
...
build:
  mkdir -p .local/bin
  GOCACHE="$PWD/.local/go-build" GOMODCACHE="$PWD/.local/go-mod" go build -o .local/bin/app .
